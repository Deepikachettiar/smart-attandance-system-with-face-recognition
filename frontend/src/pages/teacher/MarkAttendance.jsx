...
export default function MarkAttendance() {
  ...
  const [frMessage, setFrMessage] = useState('Waiting for sensor...');
  ...
  // Polling for Face Recognition + Sensor Values
  const startPolling = () => {
    pollRef.current = setInterval(async () => {
      try {
        const { data } = await py.status();
        setFrMessage(data.message || 'Waiting for sensor...');
