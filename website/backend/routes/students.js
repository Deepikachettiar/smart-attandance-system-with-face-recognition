...
router.get('/', authenticate, requireTeacher, async (req, res) => {
  try {
    ...
    const students = snap.docs
      .map(d => ({ id: d.id, name: d.data().name, email: d.data().email, studentId: d.data().studentId, createdAt: d.data().createdAt }))
...
router.get('/:id/subjects', authenticate, async (req, res) => {
  try {
    ...
    const enrollSnap = await db.collection(collections.ENROLLMENTS)
      .where('studentId', '==', req.params.id).get();
    ...
    for (const sid of subjectIds) {
      ...
      if (sDoc.exists) {
        ...
        subjects.push({
          id: sDoc.id, ...s,
          teacherName: tDoc.exists ? tDoc.data().name : 'Unknown',
        });
      }
    }
    res.json(subjects);
