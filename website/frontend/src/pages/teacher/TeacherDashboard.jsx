...
export default function TeacherDashboard() {
  ...
  useEffect(() => {
    Promise.all([api.getTeacherSubjects(), api.getAllStudents()])
      .then(async ([subs, studs]) => {
        setSubjects(subs.data);
        setStudents(studs.data);
        const map = {};
        await Promise.all(subs.data.map(async s => {
          try { map[s.id] = (await api.getAnalytics(s.id)).data; } catch {}
        }));
        setAnalytics(map);
  ...
  const avgPct = subjects.length
    ? Math.round(Object.values(analytics).reduce((a, d) => {
        const arr = d?.stats || [];
  ...
  // All at-risk students across all subjects
  const atRiskRows = Object.values(analytics).flatMap(d =>
    (d?.stats || []).filter(s => s.percentage < 75)
      .map(s => ({ ...s, subjectName: d.subject?.name, subjectCode: d.subject?.code }))
  ).slice(0, 6);
              ...
              <tbody>
                {atRiskRows.map((r, i) => (
                  <tr key={i}>
                    ...
                    <td><span style={{ fontFamily:'JetBrains Mono, monospace', fontSize:'0.78rem', color:'var(--text-muted)' }}>{r.roll_no}</span></td>
