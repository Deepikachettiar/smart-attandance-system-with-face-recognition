...
import {
  ScanFace, LogOut, LayoutDashboard, BookOpen,
  Users, BarChart2, Upload, CalendarCheck,
} from 'lucide-react';
...
const studentLinks = [
  ...
  { to: '/student/attendance', end: false, icon: BookOpen,        label: 'My Records' },
];
...
export default function Sidebar() {
  ...
  return (
    <aside style={{
      ...
      {/* Logo */}
      <div style={{ padding: '1.25rem 1.25rem', borderBottom: '1px solid var(--border)', display:'flex', alignItems:'center', gap:10 }}>
        <div style={{ width:32, height:32, background:'var(--jade)', borderRadius:8, display:'flex', alignItems:'center', justifyContent:'center' }}>
          <ScanFace size={17} color="#fff" />
        </div>
        <div>
          <div style={{ fontFamily:'Playfair Display, serif', fontWeight:700, fontSize:'0.95rem', lineHeight:1.2 }}>AttendAI</div>
      ...
      <nav style={{ flex:1, padding:'0.75rem', display:'flex', flexDirection:'column', gap:2 }}>
        {links.map(({ to, end, icon: Icon, label }) => (
          <NavLink key={to} to={to} end={end} style={({ isActive }) => ({
            ...
            borderRadius:8, textDecoration:'none', fontSize:'0.853rem', fontWeight:500,
            transition:'all 0.15s',
            ...(isActive
              ? { background:'var(--jade-bg)', color:'var(--jade2)', border:'1px solid var(--jade-border)' }
      ...
      <div style={{ padding:'0.75rem', borderTop:'1px solid var(--border)' }}>
        <div style={{
          ...
          <div style={{
            ...
            fontSize:'0.8rem', fontWeight:700, color:'var(--jade2)',
          }}>
            {user?.name?.[0]}
          </div>
